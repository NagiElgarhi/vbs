...
sudo ufw enable`,
              explanations: [
                { lines: "2", explanation: "هذا هو حجر الزاوية. نحن نأمر حارس الأمن الافتراضي لدينا برفض أي شخص يحاول الدخول عبر أي باب (`deny incoming`). هذا هو تطبيقنا المباشر لسياسة 'الرفض الاftراضي'." },
...
[ 3] 443/tcp                    ALLOW IN    Anywhere`},
    ...
    },
    {
        ...
        sections: [
            { id: "p1_c4_s1", icon: "🕒", title: "المستوى 15: إعداد المنطقة الزمنية ومزامنة الوقت (NTP)", 
              content: [
                { type: ContentType.PARAGRAPH, text: "قد يبدو إعداد الوقت أمرًا تافهًا، ولكنه في الواقع أحد أهم خطوات تكوين الخادم. السجلات غير المتزامنة من خوادم مختلفة تجعل تصحيح الأخطاء كابوسًا. المهام المجدولة (Cron jobs) التي تعمل في أوقات غير متوقعة يمكن أن تسبب فوضى. شهادات SSL/TLS تعتمد على طوابع زمنية دقيقة لتكون صالحة. باختصار، الوقت غير الصحيح يكسر كل شيء بطرق خفية ومحبطة." },
                { type: ContentType.HEADING4, text: "ورشة عمل: استخدام `timedatectl`"},
                { type: ContentType.PARAGRAPH, text: "في أنظمة لينكس الحديثة، الأداة القياسية لإدارة الوقت هي `timedatectl`."},
                { type: ContentType.CODE_EXPLANATION, language: "bash", codeTitle: "إدارة الوقت", code: `# 1. تحقق من الحالة الحالية
timedatectl

# 2. اعرض قائمة بجميع المناطق الزمنية المتاحة
timedatectl list-timezones

# 3. اضبط المنطقة الزمنية الصحيحة
sudo timedatectl set-timezone 'Asia/Riyadh'

# 4. تأكد من تمكين مزامنة NTP
sudo timedatectl set-ntp true`, explanations: [
                    { lines: "2", explanation: "يعرض لك هذا الأمر كل ما تحتاج إلى معرفته: الوقت المحلي، والوقت العالمي (UTC)، والمنطقة الزمنية، وما إذا كانت مزامنة وقت الشبكة (NTP) نشطة." },
                    { lines: "5", explanation: "هذا يعرض قائمة طويلة يمكنك البحث فيها باستخدام `grep` للعثور على منطقتك الزمنية (على سبيل المثال, `timedatectl list-timezones | grep Riyadh`)." },
                    { lines: "8", explanation: "استبدل 'Asia/Riyadh' بالمنطقة الزمنية الصحيحة لك. من الأفضل دائمًا استخدام منطقة جغرافية محددة بدلاً من الاختصارات مثل `EST`." },
                    { lines: "11", explanation: "NTP (Network Time Protocol) هو ما يحافظ على دقة ساعة خادمك. هذا الأمر يضمن أن خدمة `systemd-timesyncd` نشطة وستقوم بمزامنة ساعة خادمك بشكل دوري مع خوادم وقت عالمية موثوقة." }
                ]},
                { type: ContentType.NOTE, title: "لماذا UTC مهم؟", text: "بينما يجب عليك تعيين المنطقة الزمنية المحلية لخادمك لتسهيل قراءة السجلات، من أفضل الممارسات أن تقوم تطبيقاتك دائمًا بتخزين الطوابع الزمنية في قاعدة البيانات بالتوقيت العالمي المنسق (UTC). هذا يتجنب كل أنواع المشاكل المعقدة المتعلقة بالتوقيت الصيفي والاختلافات بين المناطق الزمنية. قم بالتحويل إلى التوقيت المحلي للمستخدم فقط عند العرض." },
              ] 
            },
            { id: "p1_c4_s2", icon: "🚫", title: "المستوى 16: تثبيت وتكوين `fail2ban` للحماية الاستباقية",
              content: [
                { type: ContentType.PARAGRAPH, text: "لقد استخدمنا `ufw limit` كخط دفاع أول ضد هجمات القوة الغاشمة. `fail2ban` هو خطوة متقدمة وأكثر قوة. إنه لا يراقب فقط محاولات الاتصال، بل يقرأ بنشاط ملفات السجلات (مثل `/var/log/auth.log`) بحثًا عن أنماط محاولات تسجيل الدخول الفاشلة المتكررة. عندما يكتشف مهاجمًا، فإنه يقوم تلقائيًا بتحديث قواعد جدار الحماية (UFW/iptables) لحظر عنوان IP الخاص بالمهاجم لفترة زمنية محددة." },
                { type: ContentType.HEADING4, text: "ورشة عمل: إعداد `fail2ban` لحماية SSH"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo apt update
sudo apt install fail2ban -y`},
                { type: ContentType.PARAGRAPH, text: "التكوين الافتراضي لـ `fail2ban` يأتي مع حماية SSH ممكّنة، ولكن من أفضل الممارسات إنشاء ملف تكوين محلي لتجاوز الإعدادات الافتراضية بدلاً من تعديل الملفات الأصلية."},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo cp /etc/fail2ban/jail.conf /etc/fail2ban/jail.local`},
                { type: ContentType.PARAGRAPH, text: "الآن، افتح الملف المحلي وقم بتخصيصه:"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo nano /etc/fail2ban/jail.local`},
                { type: ContentType.CODE_EXPLANATION, language: "ini", codeTitle: "تعديلات على jail.local", code: `[sshd]
# To enable sshd, override this in jail.local and describe your backend
backend = auto
enabled = true
port    = ssh,2222
#action = %(action_)s
maxretry = 3
findtime = 5m
bantime = 1h`, explanations: [
                    { lines: "1", explanation: "ابحث عن قسم `[sshd]`." },
                    { lines: "4", explanation: "تأكد من أن `enabled = true`." },
                    { lines: "5", explanation: "الأهمية القصوى! إذا قمت بتغيير منفذ SSH الخاص بك، فيجب عليك إضافته هنا. `fail2ban` سيراقب الآن كلاً من 'ssh' (المنفذ 22) والمنفذ 2222." },
                    { lines: "7-9", explanation: "هذه هي الإعدادات الرئيسية. `maxretry = 3`: احظر بعد 3 محاولات فاشلة. `findtime = 5m`: إذا حدثت هذه المحاولات الثلاث في غضون 5 دقائق. `bantime = 1h`: احظر عنوان IP لمدة ساعة واحدة." }
                ]},
                { type: ContentType.PARAGRAPH, text: "احفظ الملف وأعد تشغيل الخدمة:"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo systemctl restart fail2ban`},
                { type: ContentType.HEADING4, text: "التحقق من الحالة"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo fail2ban-client status sshd`},
                { type: ContentType.PARAGRAPH, text: "سيعرض لك هذا الأمر قائمة بعناوين IP المحظورة حاليًا. `fail2ban` هو مثال مثالي على الدفاع النشط. إنه يراقب سلوك المهاجم ويتخذ إجراءات تلقائية لحماية خادمك." },
              ]
            },
            { id: "p1_c4_s3", icon: "⏰", title: "المستوى 17: فهم وتأمين `cron` لجدولة المهام",
              content: [
                { type: ContentType.PARAGRAPH, text: "`cron` هو برنامج جدولة مهام كلاسيكي يعتمد على الوقت في أنظمة يونكس. إنه يعمل كخدمة في الخلفية ويقوم بتشغيل الأوامر أو النصوص البرمجية في أوقات محددة. إنه الأداة المثالية لأتمتة مهام الصيانة الروتينية مثل النسخ الاحتياطي، وتنظيف الملفات المؤقتة، وتشغيل نصوص التحديث." },
                { type: ContentType.HEADING4, text: "تحرير Crontab"},
                { type: ContentType.PARAGRAPH, text: "لكل مستخدم ملف `crontab` خاص به يمكنك تحريره باستخدام:"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `crontab -e`},
                { type: ContentType.HEADING4, text: "تشريح بناء جملة Cron"},
                { type: ContentType.PARAGRAPH, text: "كل سطر في `crontab` يمثل مهمة ويحتوي على جزأين: جدول زمني وأمر."},
                { type: ContentType.PREFORMATTED_TEXT, text: `# ┌───────────── minute (0 - 59)
# │ ┌───────────── hour (0 - 23)
# │ │ ┌───────────── day of month (1 - 31)
# │ │ │ ┌───────────── month (1 - 12)
...
# │ │ │ │ │
# * * * * *  command to execute`},
                { type: ContentType.HEADING4, text: "دراسة حالة: جدولة نص برمجي للنسخ الاحتياطي"},
                { type: ContentType.PARAGRAPH, text: "لنفترض أن لدينا نصًا برمجيًا للنسخ الاحتياطي في `/home/nagi/scripts/backup.sh`. نريد تشغيله كل ليلة في الساعة 2:30 صباحًا."},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `30 2 * * * /home/nagi/scripts/backup.sh`},
                { type: ContentType.NOTE, title: "سر المحترفين: لماذا تفشل مهام Cron؟", text: "السبب الأكثر شيوعًا لفشل مهمة `cron` هو أنها لا تعمل بنفس بيئة الـ shell التي تعمل بها. على وجه التحديد، متغير البيئة `PATH` محدود جدًا. إذا كان النص البرمجي الخاص بك يستدعي أمرًا مثل `pg_dump`، فقد لا يتمكن `cron` من العثور عليه. الحل دائمًا هو استخدام المسارات المطلقة الكاملة لجميع الأوامر داخل نصوصك البرمجية، أو تحديد متغير `PATH` في بداية ملف `crontab` الخاص بك." },
              ] 
            },
            { id: "p1_c4_s4", icon: "🤖", title: "المستوى 18: تثبيت `unattended-upgrades` للتحديثات الأمنية التلقائية",
              content: [
                { type: ContentType.PARAGRAPH, text: "تحديث خادمك يدويًا أمر جيد، ولكن من السهل أن تنسى. يمكن أن تظهر ثغرة أمنية حرجة بينما أنت نائم أو في إجازة. `unattended-upgrades` هي الحزمة الرسمية في أوبونتو لأتمة هذه العملية، مما يضمن تطبيق تصحيحات الأمان الحاسمة في أسرع وقت ممكن دون أي تدخل منك." },
                { type: ContentType.HEADING4, text: "ورشة عمل: الإعداد والتكوين"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `sudo apt install unattended-upgrades apt-listchanges -y`},
                { type: ContentType.PARAGRAPH, text: "الآن، لنقم بتكوينها. الملف الرئيسي هو `/etc/apt/apt.conf.d/50unattended-upgrades`. افتحه وتأكد من أن هذه الأسطر غير معلقة:"},
                { type: ContentType.CODE_EXPLANATION, language: "text", codeTitle: "تكوين 50unattended-upgrades", code: `Unattended-Upgrade::Allowed-Origins {
        "\${distro_id}:\${distro_codename}-security";
//      "\${distro_id}:\${distro_codename}-updates";
//      "\${distro_id}:\${distro_codename}-proposed";
//      "\${distro_id}:\${distro_codename}-backports";
};`, explanations: [
                    { lines: "2", explanation: "هذا هو الإعداد الأكثر أهمية. بشكل افتراضي، يقوم فقط بتمكين مستودع 'security'. هذا يعني أنه سيقوم تلقائيًا بتثبيت التحديثات الأمنية الحرجة فقط، وليس كل تحديث حزمة بسيط. هذا هو السلوك الأكثر أمانًا وموصى به." },
                ]},
                { type: ContentType.PARAGRAPH, text: "أخيرًا، نحتاج إلى تمكينها عن طريق إنشاء ملف التكوين النهائي:"},
                { type: ContentType.CODE_BLOCK, language: "bash", code: `echo -e 'APT::Periodic::Update-Package-Lists "1";\\nAPT::Periodic::Unattended-Upgrade "1";' | sudo tee /etc/apt/apt.conf.d/20auto-upgrades`},
                { type: ContentType.PARAGRAPH, text: "هذا يخبر `apt` بالتحقق من وجود تحديثات وتثبيتها تلقائيًا مرة واحدة يوميًا. يمكنك التحقق من السجلات في `/var/log/unattended-upgrades/`."},
              ]
            },
            { id: "p1_c4_s5", icon: "🛡️", title: "المستوى 19: مقدمة إلى AppArmor و SELinux",
              content: [
                { type: ContentType.PARAGRAPH, text: "AppArmor (المستخدم في أوبونتو) و SELinux (المستخدم في RHEL/CentOS) هما أنظمة تحكم في الوصول الإلزامية (Mandatory Access Control - MAC). إنهما خطوة أبعد من أذونات المستخدم/المجموعة/الآخرين القياسية. MAC يفرض سياسات على مستوى النواة تحدد بالضبط ما يمكن لكل برنامج القيام به." },
                { type: ContentType.PARAGRAPH, text: "على سبيل المثال، حتى لو كان Nginx يعمل كمستخدم `root` (وهو لا يفعل)، يمكن لملف تعريف AppArmor الخاص به أن يمنعه من كتابة الملفات في أي مكان باستثناء `/var/log/nginx` و `/tmp`. إذا تمكن مهاجم من استغلال ثغرة في Nginx، فإن AppArmor سيحد بشكل كبير من الضرر الذي يمكن أن يحدثه." },
                { type: ContentType.CODE_BLOCK, language: "bash", code: `# تحقق من حالة AppArmor
sudo aa-status`},
                { type: ContentType.NOTE, title: "لماذا هو مهم؟", text: "في هذه المرحلة، لا تحتاج إلى كتابة ملفات تعريف AppArmor الخاصة بك. الشيء المهم هو أن تعرف أنه موجود ويعمل بصمت في الخلفية، ويوفر طبقة حيوية من الدفاع في العمق. إنه أحد الأسباب التي تجعل تشغيل التطبيقات في حاويات (التي تستخدم AppArmor/SELinux) أكثر أمانًا." },
              ] 
            },
        ]
    },
    {
        ...
        sections: [
            { id: "p1_c5_s1", icon: "📈", title: "المستوى 20: فهم ومراقبة استخدام موارد النظام (top, htop, free)", 
              content: [
                { type: ContentType.PARAGRAPH, text: "القدرة على رؤية ما يفعله خادمك في الوقت الفعلي أمر بالغ الأهمية لتشخيص مشاكل الأداء. لينكس يأتي مع أدوات قوية لهذا الغرض." },
                { type: ContentType.HEADING4, text: "`htop`: لوحة القيادة الخاصة بك" },
                { type: ContentType.PARAGRAPH, text: "`htop` هو نسخة محسنة من `top` الكلاسيكي. إنه أكثر سهولة في القراءة ويوفر واجهة تفاعلية. قم بتثبيته باستخدام `sudo apt install htop` وشغّله." },
                { type: ContentType.PREFORMATTED_TEXT, text: `  1  [|||||         10.1%]   Tasks: 42, 1 thr; 1 running
  2  [||            4.0%]   Load average: 0.10 0.05 0.01 
  Mem[||||||||||235/981MB]   Uptime: 10 days, 21:03:01
  Swp[|           0/255MB]

  PID USER      PRI  NI  VIRT   RES   SHR S CPU% MEM%   TIME+  Command
 1234 nagi       20   0  512M  100M   12M S  5.0 10.0  1:30.50 node index.js`},
                { type: ContentType.PARAGRAPH, text: "**دراسة حالة:** تطبيق Node.js يستخدم 100% من وحدة المعالجة المركزية. باستخدام `htop`، يمكنك فرز العمليات حسب استخدام وحدة المعالجة المركزية (بالضغط على F6) وتحديد العملية المخالفة على الفور." },
                { type: ContentType.HEADING4, text: "`free -h`: التحقق من الذاكرة" },
                { type: ContentType.PARAGRAPH, text: "يعرض هذا الأمر استخدام الذاكرة بتنسيق يمكن قراءته بواسطة الإنسان. **سر المحترفين:** لا تنظر فقط إلى عمود `free`. انظر إلى عمود `available`. `available` هو تقدير لمقدار الذاكرة المتاحة لبدء تطبيقات جديدة، دون الحاجة إلى التبديل (swapping). هذا هو الرقم الأكثر أهمية." },
              ]
            },
            { id: "p1_c5_s2", icon: "📜", title: "المستوى 21: تحليل سجلات النظام الأساسية مع `journalctl`",
              content: [
                { type: ContentType.PARAGRAPH, text: "`systemd`، مدير الخدمات، لديه أيضًا نظام تسجيل مركزي يسمى `journald`. يلتقط السجلات من النواة، والخدمات، والمزيد. `journalctl` هي الأداة للاستعلام عنه." },
                { type: ContentType.HEADING4, text: "ورشة عمل: تصحيح أخطاء خدمة Nginx فاشلة" },
                { type: ContentType.PARAGRAPH, text: "لنفترض أن `sudo systemctl status nginx` يظهر `failed`." },
                { type: ContentType.CODE_EXPLANATION, language: "bash", codeTitle: "أوامر journalctl", code: `# عرض سجلات خدمة nginx فقط
sudo journalctl -u nginx.service

# متابعة السجلات في الوقت الفعلي
sudo journalctl -u nginx.service -f

# عرض آخر 100 سطر وإظهار المزيد من التفاصيل
sudo journalctl -u nginx.service -n 100 -o verbose`, explanations: [
                    { lines: "2", explanation: "هذا سيظهر لك جميع السجلات المتعلقة بـ Nginx، مما يسهل العثور على رسالة الخطأ التي تسببت في الفشل (على سبيل المثال, خطأ في بناء الجملة في ملف التكوين)." },
                    { lines: "5", explanation: "رائع لمشاهدة ما يحدث أثناء محاولتك لبدء الخدمة." },
                    { lines: "8", explanation: "يعرض لك كل حقل من حقول السجل بتنسيق مفصل." }
                ]},
              ]
            },
            { id: "p1_c5_s3", icon: "📧", title: "المستوى 22: إعداد تنبيهات البريد الإلكتروني لأحداث `sudo`",
              content: [
                { type: ContentType.PARAGRAPH, text: "معرفة متى يستخدم شخص ما `sudo` على خادمك هو إجراء أمني جيد. يمكننا إعداد تنبيه بسيط عبر البريد الإلكتروني." },
                { type: ContentType.CODE_EXPLANATION, language: "bash", codeTitle: "نص برمجي لتنبيهات sudo", code: `#!/bin/bash
# /usr/local/bin/sudo_alert.sh

# استخدم journalctl للعثور على أحداث sudo في الدقيقة الماضية
OUTPUT=$(journalctl _COMM=sudo --since "1 minute ago")

if [[ -n "$OUTPUT" ]]; then
    # أرسل بريدًا إلكترونيًا إذا تم العثور على شيء ما
    echo "$OUTPUT" | mail -s "Sudo Alert on $(hostname)" your-email@example.com
fi`, explanations: [
                    { lines: "4", explanation: "`_COMM=sudo` يقوم بتصفية سجلات `journald` للعمليات التي اسمها `sudo`." },
                    { lines: "7-10", explanation: "إذا لم يكن الإخراج فارغًا، فإنه يرسله بالبريد الإلكتروني. ستحتاج إلى إعداد MTA مثل `postfix` أو `ssmtp` ليعمل هذا." }
                ]},
                { type: ContentType.PARAGRAPH, text: "الآن، قم بإضافة هذا النص البرمجي إلى `crontab` الخاص بـ `root` ليعمل كل دقيقة: `* * * * * /usr/local/bin/sudo_alert.sh`." },
              ] 
            },
            { id: "p1_c5_s4", icon: "🤔", title: "المستوى 23: استراتيجيات النسخ الاحتياطي: ما الذي يجب نسخه؟",
              content: [
                { type: ContentType.PARAGRAPH, text: "خطة النسخ الاحتياطي الجيدة تبدأ بمعرفة ما هو مهم وما هو غير مهم." },
                { type: ContentType.HEADING4, text: "ما يجب نسخه:" },
                { type: ContentType.LIST_UNORDERED, items: [
                    "<strong>بيانات التطبيق:</strong> هذا هو الأهم. `/var/www`, `/home/nagi/projects`.",
                    "<strong>ملفات التكوين:</strong> `/etc` (خاصة `/etc/nginx`, `/etc/ssh`, `/etc/ufw`).",
                    "<strong>نسخ قواعد البيانات الاحتياطية:</strong> `pg_dump` من PostgreSQL.",
                    "<strong>بيانات المستخدم:</strong> `/home`."
                ]},
                { type: ContentType.HEADING4, text: "ما لا يجب نسخه (يمكن إعادة تثبيته):" },
                { type: ContentType.LIST_UNORDERED, items: [
                    "<strong>ملفات النظام:</strong> `/bin`, `/usr`, `/lib`.",
                    "<strong>الملفات المؤقتة:</strong> `/tmp`.",
                    "<strong>ذاكرة التخزين المؤقت:</strong> `/var/cache`."
                ]},
              ]
            },
            { id: "p1_c5_s5", icon: "💾", title: "المستوى 24: إنشاء أول نسخة احتياطية يدوية باستخدام `tar`",
              content: [
                { type: ContentType.PARAGRAPH, text: "`tar` (Tape Archive) هي الأداة الكلاسيكية لتجميع الملفات. لنقم بإنشاء نص برمجي قوي للنسخ الاحتياطي." },
                { type: ContentType.CODE_EXPLANATION, language: "bash", codeTitle: "/home/nagi/scripts/backup.sh", code: `#!/bin/bash
set -e
...
sudo -u postgres pg_dumpall -U postgres > "$DB_DUMP_FILE"
...
echo "Backup complete: $FILENAME"`, explanations: [
                    { lines: "11", explanation: "نستخدم `pg_dumpall` لإنشاء نسخة احتياطية من جميع قواعد البيانات." },
                    { lines: "15-22", explanation: "`-c` (create), `-z` (gzip), `-f` (file). نستخدم `--exclude` لتخطي الدلائل غير الضرورية. نقوم بتضمين الدلائل المهمة بالإضافة إلى نسخة قاعدة البيانات." },
                    { lines: "28", explanation: "هذا السطر ذكي. `ls -tp` يسرد الملفات حسب وقت التعديل. `tail -n +8` يأخذ كل شيء بعد الملف السابع. `xargs` يقوم بحذفها. هذا يضمن أننا نحتفظ دائمًا بآخر 7 نسخ احتياطية." }
                ]},
                { type: ContentType.NOTE, title: "الخطوة التالية: التخزين الخارجي", text: "النسخة الاحتياطية على نفس الخادم أفضل من لا شيء، لكنها لا تحميك من فشل القرص. يجب عليك نسخ هذه الأرشيفات بانتظام إلى موقع خارجي (off-site) مثل Amazon S3 أو Backblaze B2 باستخدام أدوات مثل `rclone`." },
              ]
            },
        ]
    }
