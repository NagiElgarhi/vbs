...
NODE_ENV="development"` },
                ...
                { type: ContentType.CODE_BLOCK, language: "bash", code: `# /etc/default/myapp-env
DATABASE_URL="postgresql://myappuser:prod_db_password@localhost/myappdb_prod"
STRIPE_API_KEY="sk_live_456xyz..."
NODE_ENV="production"` },
                { type: ContentType.PARAGRAPH, text: "يجب عليك تأمين هذا الملف بتقييد أذونات القراءة بحيث لا يمكن قراءته إلا من قبل المستخدم `root` والمستخدم الذي يشغل تطبيقك (`nagi`): `sudo chmod 640 /etc/default/myapp-env`. بعد ذلك، في ملف خدمة `systemd` الخاص بتطبيقك (الذي سننشئه في الفصل التالي)، يمكنك ببساطة إضافة توجيه واحد:" },
...
WantedBy=multi-user.target`, explanations: [
                    { lines: "1-4", explanation: "قسم **[Unit]** يصف الوحدة وعلاقتها بالخدمات الأخرى. `Description` هو وصف مقروء للبشر. `After=network.target` هو توجيه مهم يضمن أن `systemd` لن يحاول بدء خدمتنا إلا بعد أن تصبح الشبكة متاحة." },
                    { lines: "6-7", explanation: "قسم **[Service]** هو قلب الملف ويحدد كيفية تشغيل الخدمة. `User` و `Group` يحددان المستخدم الذي ستعمل به العملية، مما يتبع مبدأ أقل الامتيازات. `WorkingDirectory` يضبط دليل العمل قبل تشغيل الأمر." },
                    { lines: "9", explanation: "`EnvironmentFile` هو توجيه `systemd` المدمج لتحميل متغيرات البيئة من ملف، وهو ما يحافظ على أمان أسرارنا." },
                    { lines: "10", explanation: "`ExecStart` هو الأمر الفعلي الذي سيتم تشغيله. من الأهمية بمكان استخدام المسارات المطلقة الكاملة لكل من Gunicorn وملف التكوين." },
                    { lines: "11", explanation: "`ExecReload` يحدد كيفية إعادة تحميل الخدمة بأمان. إرسال إشارة HUP إلى Gunicorn يجعله يعيد تحميل عماله بسلاسة دون توقف." },
                    { lines: "14-15", explanation: "`Restart=on-failure` يخبر `systemd` بإعادة تشغيل الخدمة تلقائيًا إذا تعطلت. `RestartSec` يحدد فترة انتظار قبل محاولة إعادة التشغيل." },
                    { lines: "18-19", explanation: "قسم **[Install]** يحدد كيف يجب تمكين الخدمة. `WantedBy=multi-user.target` يربط خدمتنا بهدف بدء التشغيل القياسي للخادم متعدد المستخدمين، مما يضمن بدء تشغيلها عند الإقلاع." }
                ]},
                { type: ContentType.HEADING4, text: "إدارة الخدمة مع `systemctl`" },
                { type: ContentType.PARAGRAPH, text: "بعد إنشاء هذا الملف، يجب أن تخبر `systemd` بوجوده. ثم يمكنك إدارته مثل أي خدمة نظام أخرى." },
                { type: ContentType.CODE_EXPLANATION, language: "bash", codeTitle: "إدارة خدمة Systemd", code: `# 1. أعد تحميل daemon systemd لقراءة الملف الجديد
sudo systemctl daemon-reload

# 2. ابدأ الخدمة الآن
sudo systemctl start myapp.service

# 3. تحقق من الحالة للتأكد من أنها تعمل
sudo systemctl status myapp.service

# 4. قم بتمكين الخدمة لتبدأ تلقائيًا عند الإقلاع
sudo systemctl enable myapp.service`, explanations: [
                    { lines: "2", explanation: "هذه الخطوة ضرورية في كل مرة تقوم فيها بإنشاء أو تعديل ملف خدمة." },
                    { lines: "5", explanation: "يبدأ العملية في الخلفية." },
                    { lines: "8", explanation: "يعرض ما إذا كانت الخدمة نشطة، وآخر بضعة أسطر من سجلاتها، وأي أخطاء." },
                    { lines: "11", explanation: "ينشئ هذا رابطًا رمزيًا ضروريًا لـ `systemd` لبدء الخدمة عند إقلاع الخادم." }
                ]},
                { type: ContentType.NOTE, title: "مركزية السجلات", text: "أحد أفضل أجزاء استخدام `systemd` هو أنه يلتقط تلقائيًا جميع المخرجات القياسية ومخرجات الخطأ من خدمتك ويوجهها إلى `journald`. هذا يعني أنه يمكنك استخدام أمر واحد قوي، `sudo journalctl -u myapp.service`، لرؤية جميع سجلات تطبيقك." },
              ]
            },
            {
              id: "p2_c5_s4",
              ...
              content: [
                { type: ContentType.PARAGRAPH, text: "حتى مع أفضل الإعدادات، ستفشل التطبيقات. القدرة على تشخيص المشاكل بسرعة هي مهارة أساسية. مفتاح هذا هو معرفة مكان البحث عن السجلات." },
                { type: ContentType.HEADING4, text: "سلسلة السجلات: من الحافة إلى الداخل" },
                { type: ContentType.PARAGRAPH, text: "عندما يفشل طلب، يجب عليك تتبع رحلته عبر النظام:" },
                { type: ContentType.LIST_UNORDERED, items: [
                    "<strong>1. سجلات Nginx (`/var/log/nginx/access.log` و `error.log`):</strong> هل وصل الطلب إلى الخادم أصلاً؟ هل رأيت خطأ 4xx أو 5xx؟ `error.log` غالبًا ما يمنحك السبب الدقيق لخطأ 502 (على سبيل المثال، 'connect() to unix:/tmp/myapp.socket failed').",
                    "<strong>2. سجلات مدير العمليات:</strong>",
                    "   - **لتطبيق Node.js/PM2:** `pm2 logs my-api` سيعرض لك سجلات التطبيق المجمعة. ابحث عن تتبعات الأخطاء (stack traces) أو رسائل الخطأ.",
                    "   - **لتطبيق Python/Systemd:** `sudo journalctl -u myapp.service -f` سيعرض لك سجلات الخدمة في الوقت الفعلي.",
                    "<strong>3. سجلات قاعدة البيانات (`/var/log/postgresql/`):</strong> إذا كان خطأ التطبيق مرتبطًا بقاعدة البيانات، فتحقق من سجلات PostgreSQL بحثًا عن أخطاء مثل 'too many connections' أو أخطاء في بناء الجملة.",
                ]},
                { type: ContentType.HEADING4, text: "دراسة حالة: تشخيص خطأ 502 Bad Gateway" },
                { type: ContentType.PARAGRAPH, text: "ترى خطأ 502 في متصفحك. هذا يعني أن Nginx تلقى الطلب، لكنه فشل في الحصول على استجابة صالحة من تطبيقك الخلفي. إليك كيفية تشخيصه:" },
                { type: ContentType.LIST_UNORDERED, items: [
                    "<strong>`tail -f /var/log/nginx/error.log`</strong>: ترى `(111: Connection refused) while connecting to upstream`. هذا يعني أن Nginx حاول الاتصال، لكن لا يوجد شيء يستمع على المقبس/المنفذ المستهدف.",
                    "<strong>`pm2 list` أو `systemctl status myapp`</strong>: ترى أن حالة التطبيق هي `errored` أو `failed`.",
                    "<strong>`pm2 logs my-api` أو `journalctl -u myapp`</strong>: في سجلات التطبيق، ترى خطأ فادحًا عند بدء التشغيل: `Error: Invalid value for DATABASE_URL environment variable`. لقد وجدت السبب الجذري. لقد ارتكبت خطأ إملائيًا في متغير البيئة."
                ]},
              ]
            },
            {
              ...
              content: [
                { type: ContentType.PARAGRAPH, text: "دعنا نلخص كل ما قمنا ببنائه في هذا الباب من خلال تتبع رحلة طلب واحد عبر نظامنا بالكامل. هذا التمرين الذهني يساعد على ترسيخ فهمك لكيفية تفاعل جميع المكونات معًا." },
                { type: ContentType.HEADING4, text: "حياة طلب HTTP" },
                { type: ContentType.LIST_UNORDERED, items: [
                    "1. يكتب المستخدم `https://your_domain.com` في متصفحه.",
                    "2. يقوم DNS بحل `your_domain.com` إلى عنوان IP العام لخادمك.",
                    "3. يرسل المتصفح طلب HTTPS إلى عنوان IP الخاص بك على المنفذ 443.",
                    "4. يستقبل **جدار حماية UFW** الطلب. يرى قاعدة تسمح بالاتصالات الواردة على المنفذ 443 ويمررها.",
                    "5. يستقبل **Nginx** الطلب على المنفذ 443. يبحث في `sites-enabled` ويجد كتلة الخادم التي تطابق `server_name`. يقوم بإنهاء اتصال SSL/TLS، ويفك تشفير الطلب.",
                    "6. يرى Nginx أن الطلب يجب أن يتم توجيهه. يقرأ توجيه `proxy_pass http://unix:/tmp/myapp.socket;`.",
                    "7. يقوم Nginx بإضافة ترويسات الوكيل (`X-Forwarded-For`, etc.) ويكتب الطلب إلى **مقبس يونكس**.",
                    "8. **Gunicorn** (أو خادم Node.js) يستمع على هذا المقبس، يقرأ الطلب، ويسلمه إلى أحد عماله.",
                    "9. يقوم **عامل التطبيق** (Python/Node.js) بتنفيذ منطقك. يقرأ ترويسات الوكيل لفهم الطلب الأصلي.",
                    "10. يحتاج التطبيق إلى بيانات. يتصل بـ **PostgreSQL** عبر `localhost:5432` (أو عبر PgBouncer).",
                    "11. **PostgreSQL** يتحقق من صحة المستخدم، وينفذ الاستعلام، ويعيد البيانات.",
                    "12. يقوم التطبيق بإنشاء استجابة HTML أو JSON ويرسلها مرة أخرى إلى Gunicorn.",
                    "13. يمرر Gunicorn الاستجابة إلى Nginx عبر المقبس.",
                    "14. يقوم Nginx بتشفير الاستجابة ويرسلها مرة أخرى إلى متصفح المستخدم عبر الإنترنت.",
                ]},
                { type: ContentType.NOTE, title: "سيمفونية منسقة", text: "كل مكون لديه دور محدد ويعمل بتناغم مع الآخرين. Nginx هو الحارس والمنسق. Gunicorn/PM2 هو مدير المصنع. PostgreSQL هو أمين المكتبة. فهم هذه الأدوار والعلاقات هو مفتاح بناء وصيانة أنظمة خلفية قوية وموثوقة." },
              ]
            }
        ]
    }
